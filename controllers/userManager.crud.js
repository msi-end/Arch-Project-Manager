  ...
  db.query(query2, (err, result, field) => {
    if (err)
      throw new errorHandler(
        500,
        "There is an Error in Usermanager.crud ln:12 data not inserting." + err
      );
  });
}
...
exports.parmanentDelete_or_InactiveUser = (req, res) => {
  ...
  console.log(req.query.deletePermanently == true);

  let query;
  if (req.query.deletePermanently) {
  ...
  db.query(query, [req.params.id], (err, result, field) => {
    ...
    res.status(200).send({ status: true, msg: `User State Updated successfully` });
  });
};
...
exports.getAttendence = (req, res) => {
  ...
  db.query(query, (err, data, field) => {
    if (!err) {
      res.status(200).send({ status: true,msg:'Attendence retrived successfully', data });
    } else {
...
exports.setAttendence = (req, res, empId) => {
  ...
  const query = `UPDATE empAttendance  SET empID=${
    req.params.id || empId
  } ,date='${date}',${monthNames[month]}='P',year='${year}');`;
