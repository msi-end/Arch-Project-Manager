...
exports.addEmployeeToProject = async (req, res) => {
  ...
  if (req.body.emid && typeof req.body.emid === "string") {
    const q = `INSERT INTO normal_project_employee (ndeal_id, npcid ,category_id, emid, dateofassign) VALUES (${ndeal_id},${Number(
      npcid
    )}, ${category_id}, ${emid}, "${assignDate}");`;
    await databaseCon.query(q, async (err1, data) => {
      if (!err1) {
        await EmailSender("add", "normal", {
          ndeal_id: ndeal_id,
    ...
    req.body.emid.forEach((el) => {
      np_emp_notify.push([el, title + "#" + el, assignDate]);
    });
...
exports.removeEmployeeToProject = async (req, res) => {
  ...
  await databaseCon.query(q, async (err1, data) => {
    if (!err1) {
      res.status(200).send(data);
      await EmailSender("remove", "normal", {
        ndeal_id: dealId,
        category_id: catId,
        emid: emid,
      });
...
exports.updateSubtaskStatus = async (req, res) => {
  ...
  databaseCon.query(q, (err, result) => {
    if (!err) {
      ...
      res.status(500).send({ msg: "not updated properly! try again later..." });
    }
  });
};
...
exports.deleteSubtask = async (req, res) => {
  ...
  databaseCon.query(q, (err, result) => {
    if (!err) {
      ...
      res.status(500).send({ msg: "not deleted! try again later..." });
    }
  });
};
  ...
  databaseCon.query(q, (err, result) => {
    if (!err) {
      res
        ...
        .send({
          status: true,
          msg: "Successfully data retrieve",
          data: result,
        });
    } else {
...
exports.getDataToUpdate = async (req, res) => {
  let q =
    req.query.type == "normal"
      ? `SELECT * FROM deals WHERE id = ${req.query.id}`
      : `SELECT * FROM single_deal WHERE sdid = ${req.query.id}`;
  await databaseCon.query(q, (err, results) => {
    if (!err) {
      res.status(200).send({ results });
    } else {
...
exports.DeleteNormalProjectData = async (req, res) => {
  ...
  await databaseCon.query(q, [req.params.id], async (err, result) => {
    if (!err) {
      ...
      res
        .status(500)
        .send({ status: false, msg: "failed to delete the Data" + err });
      console.log(err);
    }
  });
};
...
exports.getNormalPhaseList = async (req, res) => {
  const q = `SELECT npf.ndeal_id as project_id, npf.task, t.task_id as id, t.task_name as name, SUM(npf.amount_got) AS total_received, MAX(npf.totalamount) AS total_amount FROM normal_projects_finance npf LEFT JOIN task t ON npf.task=t.task_id WHERE npf.ndeal_id = ? GROUP BY npf.task, t.task_id, t.task_name;  `;
  await databaseCon.query(q, [req.params.id], async (err, result) => {
    if (!err) {
